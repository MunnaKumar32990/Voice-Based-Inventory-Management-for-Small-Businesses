...
class UnitService:
    ...
    def is_valid_unit(self, product: dict, unit: str) -> bool:
        ...
        if isinstance(conversions, dict):
            ...
            if any(c.get("from_unit") == unit for c in conversions):
                return True

        # Standard Indian retail conversions (product-independent)
        if unit in ("g", "quintal", "bag", "bora", "bori", "packet") and base_unit == "kg":
            return True
        ...
        if unit == "litre" and base_unit == "ml":
            return True
        if unit in ("dozen", "box", "packet") and base_unit == "piece":
            return True
        return False
    ...
    def convert_to_base_unit(self, product: dict, quantity: Decimal, unit: str):
        ...
        if unit in ("bag", "bora", "bori") and base_unit == "kg":
        ...
        # Packaging fallback for grocery retail items (packet, bottle, piece, box)
        if unit in ("packet", "piece", "box", "bottle", "can", "bundle"):
            return quantity, unit
